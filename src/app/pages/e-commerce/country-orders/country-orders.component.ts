...
@Component({
  selector: 'lms-country-orders',
  styleUrls: ['./country-orders.component.scss'],
  template: `
    <nb-card [size]="breakpoint.width >= breakpoints.md ? 'medium' : 'giant'">
      <nb-card-header>Country Orders Statistics</nb-card-header>
      <nb-card-body>
        <lms-country-orders-map (selectEvent)="selectCountryById($event)"
                                countryId="USA">
        </lms-country-orders-map>
        <lms-country-orders-chart [countryName]="countryName"
                                  [data]="countryData"
                                  [labels]="countriesCategories"
                                  maxValue="20">
        </lms-country-orders-chart>
      </nb-card-body>
    </nb-card>
  `,
})
